...
def hA(W,points):
    '''
    Linear fit of convective heat transfer coefficient,
    ...
    coeffs = np.polyfit([p.x for p in points],[p.y for p in points], deg = 1)
    return np.polyval(coeffs,W)

def are_parameters(hA_param_set:str,
                   alpha_param_set:str, 
                   replace: list = None):
    ...
    # weights (from OpenMC)
    k_f1 =      0.892/2  # fractional power generation (fuel)
    k_f2 =      0.892/2  # fractional power generation (fuel)
    k_m =       0.077    # fractional power generation (beryllium)
    k_inc =     0.030   # fractional power generation (inconel)
    k_c1 =      0.001/2   # fractional power generation (inconel)
    k_c2 =      0.001/2   # fractional power generation (inconel)
    ...
    rho_c = coolant_density(F_to_K(1280)) # ORNL-1845 pg. 113
    rho_f = fuel_density(F_to_K(1130)) #  ORNL-1845 pg. 113
    ...
    elif hA_param_set == 'leastsq':
        # estimated_params = [0.0169877,  0.00501597, 0.00060885, 0.00862134, 0.00350632, 
        #   0.01010759, 0.00213548, 0.00570137, 0.07343354, 0.00174456, 
        #   0.01248193]
        estimated_params = [0.01856169, 0.00586621, 0.00050232, 0.00831292, 0.00341875, 0.00908601,
                            0.00188498, 0.00604178, 0.08786169, 0.00192503, 0.01573085]
    ...
    for var_name, var_value in local_vars.items():
        if var_name not in ['replace', 'p']:
            p[var_name] = var_value

    if replace is not None:
        for param, val in replace:
            p[param] = val
